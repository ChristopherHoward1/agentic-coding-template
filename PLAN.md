...
- **Decomposition unexercised under real parallelism (gates Increment 1's premise).** Milestone 11 adopted the decomposition convention but validated it only on a single serial change; no genuinely parallel batch (file-disjoint concurrent issues, multiple concurrent runs) has run under it, so the convention's core claim is untested in use. Increment 1's justification rests on N× PR-filing cost under parallelism, but N is still 1 in practice — the amplification is projected, not measured. Open question for the Product Owner: run one real parallel batch (manual triggers, manual PR filing) before building Increment 1 — which would both discharge Milestone 11's owed validation and replace forecast-N with measured-N — or accept the forecast and accelerate. This is a conscious sequencing choice, not a settled one.
