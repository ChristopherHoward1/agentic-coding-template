...
**Milestone 13: Applied AI & Data Science profile (first productization step) — active.** Taking up the first project-type profile, decomposed into two serialized implementation issues with deliberately different lifetimes:

- **PR-base** — permanent, surgical CLAUDE.md primitives (investigation-as-work-type; behavioral acceptance criteria; investigation stopping condition; domain-agnostic standards; profiles-compose; self-evaluation-is-not-independent-validation). These are inherited by every future profile.
- **PR-profile-skeleton** (`Depends on` PR-base) — a **provisional, explicitly unvalidated** `profiles/applied-ai-data-science.md`: a 4-state lifecycle-as-menu (Frame / Investigate / Implement / Operate), an analysis-only specialist-lens menu, the stopping rule as Investigate's exit condition, notebook strategy, and an ARCHI conditional-section menu with **online-vs-offline forced as a classification input**. Its sections are hypotheses the hand-pass may cut, not merely fill.

The footprints are file-disjoint (CLAUDE.md vs. a new file) but carry an interface dependency (the profile composes with the base primitives), so the two issues are **serialized**, not parallel. The specialist lenses are analysis-only: anything producing code crosses the single blessed handoff to the external Software Engineer — there is no parallel implementation channel.

**Validation criterion.** The profile is validated by a real hand-pass on the posing-coach project (offline video processing, not real-time). To guard against overfitting to the personal project portfolio, it must also be checked against **at least one conventional supervised-learning project and one LLM/RAG project** before any profile section is treated as settled.
