...
- **Increment 2 — Label-triggered agent runs.** Depends only on the existing `trigger-agent.sh` (never on Increment 1); applying a GitHub label would fire `trigger-agent.sh` in place of a manual command. Gated acceleration override; **not auto-promoted** into the active slot — taking it up is a fresh Product Owner decision. **Precondition attached to this edge:** `trigger-agent.sh` is worktree-incompatible — its `[[ -d .git ]]` repo-root check fails inside a git worktree, where `.git` is a file (n=1, found in Milestone 12, which therefore needed separate full clones for isolation). Because Increment 2 fires that exact script via label and is the increment most likely to drive concurrent runs, this incompatibility is a precondition to confront if and when Increment 2 is taken up; the narrow fix is to accept a `.git` file as well as a directory.
...
- **`trigger-agent.sh` is worktree-incompatible** — recorded as a precondition attached to **Increment 2's dependency edge** in the Acceleration Roadmap (Increment 2 fires that exact script), not as a free-standing item. n=1, gated, not yet justified.
